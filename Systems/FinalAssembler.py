...
def intermediate_file():
    path = os.path.abspath("/home/waheeb/Desktop/Scripts/Systems/inputprogram.txt")
    i_lines = []
...
def opcode_format3(instruction, operand,pc):
    ...
    if operand[0] == '#': #i = 1
        ...
        if operand.isdigit():
            ...
            return f"{opcode:02X}{flags[2:]}{int(SYMTable[operand]):03X}"
...
def opcode_format4(instruction, operand):
    ...
    if operand[0] == '#': #i = 1
        ...
        opcode += 0x1
        if operand.isdigit():
            return f"{opcode:02X}{flags[2:]}{int(operand):05d}"
        else:
        ...
        for line in file:
            ...
            if "||" in line:
                ...
                if currT == "":
                    ...
                    currSize = address - start_currT +3
                    if currSize > 30:
